...
fn main() {
    println!("Please choose a prompt (1 is easy, 2 is medium, 3 is difficult)");
    ...
    if check_mistakes.diffs.len() == 1 {
        mistakes = 0; 
    } else {
        mistakes = ((check_mistakes.diffs.len() + 2 - 1) / 2) - 1;
    }

    let wpm: f64 = ((response.as_str().chars().count() / 5) - mistakes) as f64 / minutes;
    println!("You typed {} characters", response.as_str().chars().count());
